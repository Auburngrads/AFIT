...
```{r echo = F, fig.align = 'center', fig.width = 3.9, fig.cap = "Estimating Pi"}
r <- 1
n <- 100
count <- 0
...
To estimate the value of pi, a Monte Carlo Simulation (MCS) is used to randomly generate XY coordinates between 0 and 1.  The Pythagorean theorem is used to determine if the coordinate pair falls within the area of the circle.
...
```{r hist_pi, fig.width = 3.9, echo = F, fig.cap = "Histogram of Pi Estimation"}
...
```{r, results = "asis", echo = F, fig.width= 4, warning=FALSE}
library(xtable)
...
```{r echo = F, fig.align = 'center', fig.width = 4, fig.cap = "Estimating Pi"}
r <- 1
n <- 100
count <- 0
ref <- matrix(0, nrow = n, ncol = 3)
...
```{r fig.align='center', fig.width = 4, echo = F, fig.cap = "Histogram of Pi Estimation"}
